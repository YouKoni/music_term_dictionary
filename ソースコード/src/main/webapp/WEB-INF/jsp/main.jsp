...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page
	import="model.Account,model.BrowseHistory,model.MusicTerm,java.util.List,java.util.Date"%>
					...
					<table class="table table-borderless">
						...
						<tbody>
							<c:forEach var="musicTerm" items="${ musicTermList }">
								<tr>
									<td>
										<!-- 検索された音楽用語・記号を表示--> <!-- 解説画面ページに移り、音楽用語・記号の情報をID情報を基に表示 -->
							...
							<c:forEach var="browseHistory" items="${ browseHistoryList }">
								...
								<c:if test="${ accountUser != null }">
									<tr>
										...
										<td>
											<!-- 解説画面ページから検索ページに移った際の音楽用語・記号を表示(ログインするアカウントによって異なる) --> <c:out
												value="${ browseHistory.termName }" />
										</td>
									</tr>
								</c:if>
							</c:forEach>
